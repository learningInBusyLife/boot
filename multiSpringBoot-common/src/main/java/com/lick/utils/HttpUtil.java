...
public class HttpUtil {
	 ...
	 * @return
	 */
    public static String UserAgentType(HttpServletRequest request) {
        String uaType = null;
        try {
            ...
            String agent = ua.toLowerCase();
            if (agent.indexOf("micromessenger") > 0) { // 微信
                uaType = "micro";
            } else if (agent.indexOf("aliapp(tb/") > 0) {// 淘宝
                uaType = "tb";
            } else if (agent.indexOf("aliapp(ap/") > 0) {// 支付宝
                uaType = "ap";
            } else if (agent.indexOf("aliapp(dingtalk/") > 0) {// 钉钉
                uaType = "dingtalk";
            } else if (agent.indexOf("aliapp(tm/") > 0) {// 天猫
                uaType = "tm";
            }
        } catch (Exception e) {
            return null;
